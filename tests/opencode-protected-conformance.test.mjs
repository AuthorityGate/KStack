...
const evidence = JSON.parse(fs.readFileSync(new URL('../.kstack/qualifications/opencode-v1.18.25-conformance-evidence.json', import.meta.url), 'utf8'));
...
test('protected OpenCode evidence is exact, operation-scoped, and contains all 20 passing executions', () => {
  const body = { ...evidence }; delete body.evidenceDigest;
  assert.equal(evidence.evidenceDigest, digest(body));
  assert.equal(evidence.sourceEvidence.binarySha256, 'd91e0d33676d0839f7cde87924cd4127ea88c9d6784eea9f009a7d08bdc60eeb');
  ...
  assert.equal(evidence.operationStatus.operationProfileId, 'opencode.native-skill-advisory.v1');
...
test('checked evidence matches the frozen ten-group positive/negative fixture registry', () => {
  const gate = validateOpenCodeDependencyGateSet(evidence.dependencyGateSet);
  assert.equal(gate.executable, true);
  assert.equal(gate.rows.every((row) => row.status === 'SATISFIED'), true);
  const fixtures = validateOpenCodeFixtureSet(evidence.fixtureSet);
  const frozen = buildOpenCodeAdvisoryFixtureSet(evidence.plan.operationProfileDigest);
...
test('all observer receipts are protected, available, non-writable, and replay to the checked executions', () => {
  const executions = new Map(evidence.executions.map((row) => [row.fixtureId, validateOpenCodeExecution(row)]));
  for (const row of evidence.adjudicationRows) {
    const receipt = validateOpenCodeObserverReceipt(row.observerReceipt);
    ...
    assert.deepEqual(adjudicateOpenCodeFixture(replayInput(row)).execution, executions.get(row.fixtureId));
  }
});
...
test('receipt forgery, native-event drift, environment drift, and status promotion cannot replay as PASS', () => {
  ...
  assert.throws(() => adjudicateOpenCodeFixture(replayInput(row, {
    observerReceipts: [{ ...row.observerReceipt, subjectWritable: true }]
  })));
  assert.throws(() => adjudicateOpenCodeFixture(replayInput(row, { nativeEventSequence: ['REPOSITORY_UNCHANGED'] })));
  assert.throws(() => adjudicateOpenCodeFixture(replayInput(row, { environmentEndDigest: `sha256:${'0'.repeat(64)}` })));
