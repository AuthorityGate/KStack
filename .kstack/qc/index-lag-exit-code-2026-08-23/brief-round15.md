# QC round 15: dedicated exit code — corrected consumer methodology (whole-file co-occurrence, real alias-pattern search)

## Scope and status

`qcFixRound = 4` (unchanged — no production code changed this round).
Round 14: **Codex reverted to revise (92, down from 98)** with one
concrete failedCheck: the round-14 methodology was flawed in two specific
ways — (1) the alias search was described in prose but its actual command
and output were never shown as evidence in the packet; (2) the
exit-status-idiom search piped results through `grep -i jira`, which only
catches a status idiom and the string "jira" occurring on the SAME LINE —
missing a file that, say, assigns an exit code to a variable on one line
and references "jira" elsewhere in the same file. Opus approved (96) with
one explicitly non-blocking question about out-of-repo distribution
artifacts (a documentation-only residual, not a behavioral concern, since
20 is a distinct nonzero code).

This round fixes both methodology gaps directly: a real, standalone
alias-pattern search with its command and output shown, and a two-step
whole-file (not same-line) co-occurrence check between exit-status idioms
and any mention of "jira."

## Fresh verification evidence (new this round — corrected methodology)
...
You are reviewing round 15 independently. You have not seen any other
reviewer's output for this round. Production code is unchanged since round
8. What's new this round is the corrected consumer-search methodology
(standalone alias-pattern search with real command+output, whole-file
rather than same-line status-idiom/"jira" co-occurrence), closing the two
methodology gaps named in round 14.
