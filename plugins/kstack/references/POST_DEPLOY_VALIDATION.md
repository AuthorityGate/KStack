...
A valid run has two required lanes:
...
`HEALTHY` requires both lanes and both configured runtime budgets. Provider success, a KStack canary alone, a
...
and evidence digests. It deliberately deletes the raw JSON reporter payload
...
budgets. On `FAILED`, it appends the receipt digest to the linked Jira work item
