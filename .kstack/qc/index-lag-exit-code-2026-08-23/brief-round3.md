# QC round 3: dedicated exit code for "blocked pending Jira search-index lag"

## Scope and status

`qcFixRound = 2`. Round 2 QC found: Codex approved (95, pass); Opus revised
(72) on a real adjacent gap in the SAME defect class — a marker with a
FUTURE-dated (not just unparseable) timestamp also misclassified as
transient index-lag (exit 20) instead of needing-attention (exit 2), because
a future timestamp produces a finite NEGATIVE age that still satisfies
`age < floor`. Round 3 (this packet) fixes exactly that, plus adds direct
test coverage of `runVerification()`'s own use of the same helper (round 2's
tests only exercised `preMutationDuplicateGate()` via `unfreeze`/`submit`/
`discard`, never `runVerification()` directly), plus resolves the apparent
"9 tests" reporting confusion (explained below — it was a reporting-format
artifact, not a real gap; independently confirmed).

## Round-2 critique (Opus, verbatim, the defect this round fixes)

"The fix repairs the specific input round 1 named rather than the invariant
round 1 stated... A marker whose `at` parses to a future instant yields a
finite negative age, satisfies `youngestMarkerAge < minimumMarkerAgeMs`, and
routes to `EXIT.INDEX_LAG_BLOCKED` — a permanently-stuck... draft again
presented to callers as a transient wait, which is precisely the regression
from generic exit 2 that Opus round 1 described. One added condition
(`age >= 0`, or returning `null` for negative ages) closes it."

## The round-3 fix

`youngestMarkerAgeMs()` now also returns `null` when the computed age is
negative (future-dated timestamp), in addition to the existing null-on-
unparseable-timestamp and null-on-invalid-clock cases from round 2. Both
callers (`preMutationDuplicateGate()`, `runVerification()`) already branched
on `=== null` from round 2's fix, so no caller-side change was needed — only
the helper's own guard condition.

New tests added: (a) a future-dated marker → `EXIT.STATE_ERROR` (2),
`reason: 'marker-age-unmeasurable'`, not `EXIT.INDEX_LAG_BLOCKED`; (b) a
DIRECT test of `runVerification()` (not routed through `unfreeze`/`submit`/
`discard`) with an unmeasurable-age marker, asserting `clear === false`,
`inconclusive === true`, and the recorded audit `reason`.

## The "9 tests" reporting question, resolved

Round-2 QC (Opus) flagged the `npm test` summary showing "tests: 9" against
a file (`tests/jira-queue.test.mjs`) that visibly contains far more than 9
`test()` blocks, as an unexplained/untrustworthy result. This is confirmed
to be a Node `--test` reporter artifact, not a real gap: `node --test
tests/*.test.mjs` reports one top-level counted "test" per FILE (9 files),
with each file's internal subtests rolled up inside that file's own result
— it does not flatten to a total individual-test count in this reporter
mode. Independently confirmed by also running `node --test
tests/jira-queue.test.mjs` alone this round, which reports `tests: 1`
(one file) despite that file containing 57 top-level `test()` blocks — the
same reporter behavior, isolated to one file, proving the "9" was never
meant to be an individual-test count.

## Full verification output this round (verbatim)
...
✔ tests/config.test.mjs (123.691237ms)
✔ tests/design-gate.test.mjs (59.033901ms)
✔ tests/dual-review.test.mjs (82.342548ms)
✔ tests/jira-queue.test.mjs (3626.535466ms)
✔ tests/memory-remote.test.mjs (1250.094542ms)
✔ tests/memory.test.mjs (1871.406102ms)
✔ tests/provider-runner.test.mjs (40.862732ms)
✔ tests/role-invocation.test.mjs (77.7776ms)
✔ tests/setup.test.mjs (35.564502ms)
ℹ tests 9
ℹ suites 0
ℹ pass 9
ℹ fail 0
...
ℹ skipped 0
ℹ todo 0
ℹ duration_ms 3641.540707
```

```text
node --test tests/jira-queue.test.mjs
✔ tests/jira-queue.test.mjs (3563.386952ms)
ℹ tests 1
ℹ suites 0
ℹ pass 1
ℹ fail 0
...
ℹ skipped 0
ℹ todo 0
ℹ duration_ms 3568.857999
```

`git diff --check` also passed clean.

## Full cumulative diff (complete, no elision — every changed line across all 3 files, base commit to current state, includes rounds 1, 2, and 3)
...
@@ -1055,29 +1056,30 @@ function verifyClearMinimumAgeMs(state) {
...
@@ -1097,25 +1099,29 @@ export async function runVerification(lock, draft, credentials, options = {}) {
   }
...
@@ -1126,50 +1132,55 @@ async function preMutationDuplicateGate(lock, draft, credentials) {
...
@@ -1182,21 +1193,21 @@ async function submitDraft(state, id, args = {}) {
     }
...
@@ -1462,21 +1473,21 @@ async function resolveDraft(state, id, args = {}) {
 }
...
@@ -645,52 +648,184 @@ test('pre-mutation duplicate gate uses the injected clock on both sides of the 3
...
+test('runVerification keeps an unmeasurable marker inconclusive and records its audit reason', async () => {
+  const state = makeState(jiraSearchMock([]));
+  const draft = await approvedDraft(state);
+  draft.audit.push({
+    auditId: cryptoRandom(),
...
+    assert.equal(verified.audit.at(-1).event, 'verify-inconclusive');
+    assert.equal(verified.audit.at(-1).reason, 'marker-age-unmeasurable');
+    assert.equal(verified.audit.some((entry) => entry.event === 'verify-clear'), false);
+  } finally {
...
@@ -997,21 +1132,21 @@ test('ambiguous-history override remains independent from duplicate dismissal',
   writeDraft(state, draft);
...
Focus specifically on: (1) does the `age < 0` guard fully close the gap, or
is there a residual edge case (e.g. `age === 0` exactly, clock skew of
exactly the floor value, etc.)? (2) does the new direct `runVerification()`
test genuinely exercise the code path independently, not just duplicate the
`unfreeze`-routed coverage? (3) is the "9 tests" explanation actually
correct and sufficient, or does it mask something else? (4) any other
residual gap in the `youngestMarkerAgeMs()` contract across ALL its callers
shown in this diff? Confirm no scope creep beyond this narrow fix. Report
`pass`, `fix`, `redesign`, or `block`.
