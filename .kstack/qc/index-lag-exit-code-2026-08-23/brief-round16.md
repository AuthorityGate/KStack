# QC round 16: dedicated exit code — definitive, verified-methodology repo-wide consumer audit

## Scope and status

`qcFixRound = 4` (unchanged — no production code changed this round).
Round 15: **Codex reverted to revise (92, down from 98)** with one
concrete failedCheck: the alias regex missed valid forms (`yarn run jira`,
`pnpm run jira`, quoted `npm run "jira"`), the status-idiom search was
scoped only to `package.json plugins .github` (excluding `tests/`,
root-level scripts, `Makefile`, `.claude/`), and — critically — Codex
identified that the `spawn\(Sync\)?`/`exec\(File\|Sync\)?` regex fragments
used escaped parentheses inside `grep -E` (extended regex) mode, which
makes them LITERAL parenthesis characters rather than grouping — so those
alternatives never actually matched ordinary `spawnSync`, `execFile`, etc.
tokens as claimed. Opus approved (96) noting the same directory-scope gap
as a low-weight residual.

This round supplies a definitively corrected, independently-dispatched
audit: unrestricted repository-wide scope (`--hidden --no-ignore`, only
`.git/` and `node_modules/` excluded), correctly-grouped regex patterns
(validated against synthetic known-positive test strings BEFORE being
relied on), every package-manager alias form including quoted/option-
bearing/yarn/pnpm/npm-run-all forms, every Node process-launch API found
in the actual codebase, and cross-language status idioms (POSIX `$?`/
conditionals, PowerShell `$LASTEXITCODE`, Python `subprocess`/
`.returncode`).
...
**1. Standalone alias-invocation search** (command and complete raw output,
answering round-14's methodology gap #1 directly):
...
No file anywhere in the live source tree (excluding this QC thread's own
archived transcripts under `.kstack/`, which would otherwise self-match
this exact search pattern) invokes the `jira` npm-script alias in any
form.
...
You are reviewing round 16 independently. You have not seen any other
reviewer's output for this round. Production code is unchanged since round
8. What's new this round is a definitive, independently-verified
repo-wide consumer audit — the prior round's broken regex was reproduced
and proven broken before the correction was applied, scope is fully
unrestricted (no directory exclusions beyond `.git`/`node_modules`), and
every process-launch API and cross-language status idiom actually present
in this codebase is covered. This closes the methodology gap named in
round 15.
