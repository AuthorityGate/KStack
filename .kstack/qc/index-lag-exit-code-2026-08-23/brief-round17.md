# QC round 17: dedicated exit code — corrected alias regex evidence closes the last methodology gap

## Scope and status

`qcFixRound = 4` (unchanged — no production code changed this round).
Round 16: **Codex reverted to revise (90, down from 98)** with one
concrete failedCheck: the round-16 packet still displayed round 11's
STALE alias regex (`npm[[:space:]]+run[[:space:]]+jira|...`) as its
"standalone alias-invocation search" evidence — that regex genuinely
misses `yarn run jira` (only matched `yarn[[:space:]]+jira` without
"run"), quoted forms (`npm run "jira"`), and option-bearing invocations,
exactly as Codex said. The round-16 dispatch had actually PERFORMED a
correctly validated, comprehensive alias search internally, but I failed
to extract and embed that specific evidence into the reviewed packet —
the stale regex from round 11 was carried forward unchanged instead.
**Opus approved this round (95)** with two items, both non-blocking:
whether any out-of-repo consumer reads the changed `marker-age-unmeasurable`
audit-reason string (same class of residual as the exit-code question,
disclosed identically), and the same out-of-repo exit-code question from
prior rounds (unobservable by any in-repo search by construction).
...
You are reviewing round 17 independently. You have not seen any other
reviewer's output for this round. Production code is unchanged since round
8. What's new this round is the corrected, synthetic-validated
alias-invocation search — closing round 16's finding that the packet had
displayed a stale, incomplete regex instead of the actually-corrected one.
Scope is fully
