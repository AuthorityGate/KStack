...
function structuredOutput(stdout) {
  ...
  const walk = (entry) => {
    ...
    if (entry.type === 'text' && typeof entry.text === 'string') texts.push(entry.text.trim());
    if (entry.type === 'toolRequest' && entry.toolCall?.value) toolRequests.push(entry.toolCall.value);
    if (entry.type === 'toolResponse' && typeof entry.toolResult?.error === 'string') toolErrors.push(entry.toolResult.error);
    for (const child of Object.values(entry)) walk(child);
  };
  walk(value);
  return { value, texts, toolRequests, toolErrors };
}

function skillsList(stdout) {
  return stdout.split('\n').slice(1).filter((line) => line.includes('|')).map((line) => {
    const [name, description, descriptionTokens, contentTokens, location] = line.split('|').map((entry) => entry.trim());
    return { name, description, descriptionTokens, contentTokens, location };
  });
}
...
if (binarySha256 !== '057a1788b48cc1452203920afd31eac274d5b9cb10026734d587e5db33885792') throw new Error('Goose binary digest mismatch');

const cellRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'kstack-goose-conformance-'));
...
const providerScript = fileURLToPath(new URL('./goose-v1.48.0-conformance-provider.mjs', import.meta.url));
...
const adapterDigest = fileDigest('plugins/kstack/scripts/kstack-goose-adapter.mjs');
const conformanceDigest = fileDigest('plugins/kstack/scripts/kstack-goose-conformance.mjs');
const providerScriptDigest = fileDigest('.kstack/qualifications/goose-v1.48.0-conformance-provider.mjs');
const pid1ReaperSourceDigest = fileDigest('.kstack/qualifications/kstack-pid1-reaper.c');
const supplyChainEvidenceDigest = fileDigest('.kstack/qualifications/goose-v1.48.0-supply-chain-2026-08-29.md');
const reproducibleBuildEvidenceDigest = fileDigest('.kstack/qualifications/goose-v1.48.0-reproducible-build-2026-08-29.md');
const profileDigest = recordDigest(GOOSE_ADAPTER_PROFILE);
const runningHostBuildDigest = recordDigest({
  product: 'goose', version: '1.48.0', binarySha256,
  releaseTag: 'v1.48.0', supplyChainEvidenceDigest, reproducibleBuildEvidenceDigest
});
...
const dependencyRows = requiredGooseDependencies('ADVISORY').map((hpItemId) => {
  ...
  return classifyGooseDependencyGate({
...
const gate = evaluateGooseDependencyGateSet({
  operationProfileId: GOOSE_ADAPTER_PROFILE.profileId, operationProfileDigest: profileDigest,
  operationFamily: 'ADVISORY', rows: dependencyRows, evaluatedAt, expiresAt
});
const fixtureSet = buildGooseAdvisoryFixtureSet(profileDigest);
const environmentFacts = {
  binarySha256, runningHostBuildDigest, adapterDigest, conformanceDigest, providerScriptDigest,
  pid1ReaperSourceDigest, supplyChainEvidenceDigest, reproducibleBuildEvidenceDigest,
  platform: `${process.platform}-${process.arch}`,
  networkInterfaces: interfaces, nativePermission: GOOSE_ADAPTER_PROFILE.nativeTools.length === 0 ? 'NO_NATIVE_TOOLS' : 'NATIVE_TOOLS_DECLARED',
  providerMode: GOOSE_ADAPTER_PROFILE.providerMode, externalPlugins: false, sessionPersistence: false,
  namespaces: {
...
const plan = sealGooseConformancePlan({
  hostId: 'goose', runningHostBuildDigest, hostExecutableIdentityDigest: digest(binaryBytes),
  adapterDigest, activeSetDigest: fileDigest('plugins/kstack/install-health-audit-manifest-v1.json'),
  policyDigest: recordDigest({ profile: GOOSE_ADAPTER_PROFILE, productionTargets: 0, credentials: 0 }),
  ...
  harnessDigest: fileDigest('.kstack/qualifications/goose-v1.48.0-conformance-child.mjs'),
...
function setupFixture(fixture) {
  ...
  const skillRoot = path.join(repositoryRoot, '.agents', 'skills', 'kstack-conformance-probe');
...
function fixtureMarker(fixture) {
  return `KSTACK_GOOSE_FIXTURE_${fixture.fixtureId.replace(/[^A-Za-z0-9]/gu, '_').toUpperCase()}`;
}
...
function operationBase(fixture, context) {
  return {
    ...
    inputDigest: sha256Hex(`KSTACK_GOOSE_CONFORMANCE\n${fixture.fixtureId}\n`),
    expectedMarker: fixtureMarker(fixture), brokerTicketDigest: null, deadlineMs: fixture.deadlineMs
  };
}

function runVersion(fixture, context) {
  const operation = { ...operationBase(fixture, context), action: 'VERSION', expectedMarker: '1.48.0', deadlineMs: 5_000 };
  const invocation = buildGooseInvocation(operation, { loopbackEndpoint: endpoint, verifyBinary: true });
  ...
  const observation = observeGooseExecution(operation, {
    exitCode: run.status, signal: run.signal, timedOut: run.status === 124, stdout: run.stdout, stderr: run.stderr,
    providerRequestCount: 0, providerRequestDigest: null, loopbackOnly: true,
    rootMutationDigest: recordDigest({ repositoryBefore: before.digest, repositoryAfter: after.digest, pathRootAfter: tree(context.pathRoot).digest }).slice(7), orphanCount: 0
  });
  return { run, before, after, observation };
}

function runDiscovery(fixture, context) {
  const operation = { ...operationBase(fixture, context), action: 'SKILLS_LIST', expectedMarker: 'kstack-conformance-probe', deadlineMs: 10_000 };
  const invocation = buildGooseInvocation(operation, { loopbackEndpoint: endpoint, verifyBinary: true });
  const before = tree(context.repositoryRoot);
  const run = runSubject(invocation.command, invocation.args, { cwd: invocation.cwd, env: invocation.env, input: invocation.stdin, timeout: invocation.deadlineMs });
  const after = tree(context.repositoryRoot);
  const rows = skillsList(run.stdout);
  const observation = observeGooseExecution(operation, {
    exitCode: run.status, signal: run.signal, timedOut: run.status === 124, stdout: run.stdout, stderr: run.stderr,
    providerRequestCount: 0, providerRequestDigest: null, loopbackOnly: true,
    rootMutationDigest: recordDigest({ repositoryBefore: before.digest, repositoryAfter: after.digest, pathRootAfter: tree(context.pathRoot).digest }).slice(7), orphanCount: 0
...
function runAdvisory(fixture, context, deadlineMs = fixture.deadlineMs) {
  const operation = {
    ...operationBase(fixture, context), action: 'ADVISORY', expectedMarker: fixtureMarker(fixture), deadlineMs,
  ...
  const after = tree(context.repositoryRoot);
  const parsed = structuredOutput(run.stdout);
  let observation = null; let observationErrorCode = null;
  try {
    observation = observeGooseExecution(operation, {
      exitCode: run.status, signal: run.signal, timedOut: run.status === 124, stdout: run.stdout, stderr: run.stderr,
...
for (const fixture of fixtureSet.fixtureSet.fixtures) {
  ...
  let subjectMutationObserved = version.before.digest !== version.after.digest;
  ...
  if (fixture.fixtureGroupId === 'identity-currentness') {
    ...
    if (fixture.polarity === 'NEGATIVE') {
      const duplicate = path.join(context.pathRoot, 'home', '.agents', 'skills', 'kstack-conformance-probe');
    ...
    subjectMutationObserved ||= discovery.before.digest !== discovery.after.digest;
    ...
    if (fixture.polarity === 'POSITIVE') {
      ...
      subjectMutationObserved ||= advisory.before.digest !== advisory.after.digest;
      passed &&= advisory.observation?.observation.nativeOutcome === 'MATCH' && advisory.parsed.texts.includes(fixtureMarker(fixture));
    ...
    subjectMutationObserved ||= discovery !== null && discovery.before.digest !== discovery.after.digest;
    ...
    const advisory = runAdvisory(fixture, context);
    subjectMutationObserved ||= advisory.before.digest !== advisory.after.digest;
    if (fixture.polarity === 'POSITIVE') passed &&= advisory.observation?.observation.nativeOutcome === 'MATCH' && advisory.parsed.texts.includes(fixtureMarker(fixture));
    else {
      const markerObserved = fs.existsSync(path.join(context.repositoryRoot, 'forbidden-native-action.txt'));
      const requestedBash = advisory.requests.length === 2 && advisory.parsed.toolRequests.some((request) => request?.name === 'bash');
      const nativeToolUnavailable = advisory.parsed.toolErrors.some((error) => /Tool 'bash' not found\. Available tools: \[\]/u.test(error));
      passed &&= requestedBash && nativeToolUnavailable && !markerObserved && advisory.before.digest === advisory.after.digest;
      details.groupEvidenceDigest = recordDigest({ requestedBash, nativeToolUnavailable, markerObserved, status: advisory.run.status, toolRequestDigest: recordDigest(advisory.parsed.toolRequests), toolErrorDigest: recordDigest(advisory.parsed.toolErrors) });
    }
    details.adapterObservationDigest = advisory.observation?.observationDigest ?? null;
    details.providerRequestDigest = recordDigest(advisory.requests);
  } else if (fixture.fixtureGroupId === 'approval-broker') {
    if (fixture.polarity === 'POSITIVE') {
      const display = GOOSE_ADAPTER_BOUNDARY.requestNativeApprovalDisplay({ operationId: fixture.fixtureId, displayArtifactDigest: '1'.repeat(64), displayNonceDigest: '2'.repeat(64) });
      const route = GOOSE_ADAPTER_BOUNDARY.routeProtectedBroker({ operationId: fixture.fixtureId, brokerTicketDigest: '3'.repeat(64), providerClass: 'SYNTHETIC_LOOPBACK_NO_CREDENTIAL' });
      const advisory = runAdvisory(fixture, context);
      subjectMutationObserved ||= advisory.before.digest !== advisory.after.digest;
      ...
      let rejected = false;
      try { GOOSE_ADAPTER_BOUNDARY.routeProtectedBroker({ operationId: fixture.fixtureId, brokerTicketDigest: '3'.repeat(64), providerClass: 'PRODUCTION_CREDENTIAL' }); } catch { rejected = true; }
      passed &&= rejected;
    ...
    else passed &&= advisory.observationErrorCode === 'KSTACK_GOOSE_ADAPTER_OUTPUT_SECRET' && advisory.before.digest === advisory.after.digest;
    details.adapterObservationDigest = advisory.observation?.observationDigest ?? null;
    details.providerRequestDigest = recordDigest(advisory.requests);
    ...
    details.groupEvidenceDigest = recordDigest({ codexBaseline: member?.sha256, claudeBaseline: member?.sha256, gooseDiscoveryDigest: recordDigest(discovery.rows), candidate, testStatus: configTest.status });
  }
  ...
  rawRows.push({
    ...
    forbiddenSideEffects: subjectMutationObserved,
...
for (const row of rawRows) {
  ...
  const sealedReceipt = sealGooseObserverReceipt({
  ...
  const execution = adjudicateGooseFixture(adjudicationInput);
...
const observerReceiptDigests = observerReceipts.map((receipt) => hostAddress('KSTACK-GOOSE-CONFORMANCE-OBSERVER-RECEIPT-V1', receipt)).sort();
...
const status = deriveGooseOperationStatus({
  operationId: 'advisory', operationProfileId: GOOSE_ADAPTER_PROFILE.profileId,
...
const report = {
  schema: 'kstack-goose-v1.48.0-protected-conformance-v1',
  sourceEvidence: { releaseTag: 'v1.48.0', binarySha256, supplyChainEvidenceDigest, reproducibleBuildEvidenceDigest },
