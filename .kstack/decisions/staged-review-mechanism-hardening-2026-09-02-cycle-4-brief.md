...
gate held it at confidence 72 with no material dissent. Its findings and this
design's answers:

- The cross-block contracts credited BLK-ISOLATION with bounding the
  provider-to-service channel while BLK-ISOLATION's own boundary disclaimed any
  provider-service claim, so the design contradicted itself about who owned the
  channel that BLK-PROTOCOL had in fact taken over. The contracts now name three
  channels and one owner each: BLK-DISCLOSURE for what the runner hands a
  process, BLK-ISOLATION for shared local provider state write-side, and
  BLK-PROTOCOL for the provider-to-service channel, which it addresses by
  removing the shared domain rather than by bounding traffic.
- Family separation was asserted without naming the assumption it rests on or
  the limit of that assumption. BLK-PROTOCOL now states the assumption — two
  separately identified vendors operate separate service-side state domains —
  states the fail-closed behaviour in each direction, and states plainly that an
  aggregator fronting both vendors would present two vendor identities over one
  domain and defeat the check undetected.
- Disposition authority was undefined: nothing said who may record one, so the
  agent under review could discharge the findings against it. Only a human role
  may now take that act. The record must name a recognised human role, name a
  person, carry an attestation time, bind to the exact final review's digest, and
  be a regular file inside the project. What the mechanism can and cannot
  establish is stated at that strength: no code path in the runner writes such a
  record and no reviewer holds a write primitive, so nothing in the workflow can
  author its own discharge, but the mechanism cannot prove authorship, for the
  same single-operator reason the evidence set is unsigned. A stronger control
  is available and is not taken unilaterally here — this repository's safety
  hook could deny the record's path to every agent tool surface, which would
  make agent authorship structurally impossible rather than merely unsupported;
  it changes a control-plane component that governs every agent session in the
  repository and is therefore proposed for the owner's decision rather than
  adopted inside this thread.
- The claim that a reviewer has no read primitive was never an asserted
  acceptance condition, and the adversarial check probed only two fixed
  filenames. The argument sequences that remove the write primitive are now a
  named set checked adjacently, and the adversarial reviewer now discovers the
  peer working directory by listing the shared output directory, attempts real
  reads of every artifact inside it and of each per-role output file, and fails
  the cycle on any successful read. The two claims are kept distinct: the probe
  establishes absence at dispatch time, the flag set establishes denial.
