# QC round 4: dedicated exit code — cheap polish pass

## Scope and status

`qcFixRound = 3`. Round 3 QC: both reviewers APPROVED the diff (Codex 97,
Opus 80), zero failed checks, zero security findings from either — the gate
only failed because combined confidence (`min(97,80)=80`) is below the 95
threshold, not because of an actual defect. Opus's round-3 recommendation
was explicitly "pass (approve as-is; the follow-ups below are cheap and
non-blocking)." This round addresses exactly Opus's cheap follow-ups, to
see whether a fresh independent pass now clears 95+ on both sides.

Also: the orchestrating session independently added an "accepted post-ship
addition" note to the frozen spec
(`.kstack/decisions/jira-ticket-queue-final-spec-2026-08-17.md`) recording
exit 20, since Opus round 3 correctly flagged that the spec's original
§4.10 exit-code inventory table doesn't mention it (it postdates the frozen
spec). Independently confirmed via grep: no other repo document (SKILL.md,
etc.) enumerates exit codes and needed updating.

## Round-3 critique addressed this round (Opus, verbatim excerpts)

- "the ternary... is now triplicated across unfreeze/submit/discard — a
  small `gateExitCode(gate)` helper would prevent the three copies
  drifting" → **done**, extracted and used at all three call sites, no
  behavior change.
- "add the future-dated marker as a third case to the direct
  `runVerification()` test so the negative-age branch has coverage on both
  callers" → **done**, the direct test now covers both unparseable AND
  future-dated markers.
- "settle the test-count question decisively once with
  `node --test --test-reporter=tap tests/jira-queue.test.mjs`" → **done**,
  confirms 57 individual test cases in that file, `node --test
  tests/*.test.mjs`'s "9" was always a per-FILE count, not a real gap.
- "Is `gate.unavailable` consumed anywhere outside the three call sites
  shown?" → **answered**, independently grepped: no runtime/test code reads
  `.unavailable`; only the three mutation call sites consume `blocked`/
  `keys`/`reason`.
- "the audit `reason` strings... are compared via raw string literals with
  no shared constant" → **addressed**, the four scoped reason strings are
  now centralized as named constants at their definition/comparison sites;
  test assertions intentionally retain literal expected values (as contract
  checks against the constants, not duplicated magic strings in production
  code).
...
✔ tests/config.test.mjs (122.43291ms)
✔ tests/design-gate.test.mjs (57.14336ms)
✔ tests/dual-review.test.mjs (81.345746ms)
✔ tests/jira-queue.test.mjs (2039.228498ms)
✔ tests/memory-remote.test.mjs (1240.65242ms)
✔ tests/memory.test.mjs (1802.575276ms)
✔ tests/provider-runner.test.mjs (39.471384ms)
✔ tests/role-invocation.test.mjs (80.135797ms)
✔ tests/setup.test.mjs (36.476537ms)
ℹ tests 9
ℹ pass 9
ℹ fail 0
```
...
```text
node --test --test-reporter=tap tests/jira-queue.test.mjs
TAP version 13
# Subtest: tests/jira-queue.test.mjs
ok 1 - tests/jira-queue.test.mjs
1..1
# tests 1
# pass 1
# fail 0
...
A supplemental `--test-isolation=none` run confirmed 57/57 individual test
cases in `tests/jira-queue.test.mjs` pass. `git diff --check` passed clean.

## Full cumulative diff (complete, no elision — base commit to current state, includes rounds 1-4)
...
This round is deliberately narrow — confirm: (1) the `gateExitCode(gate)`
extraction is behavior-preserving at all three call sites (no logic
change, just deduplication); (2) the new future-dated-marker case in the
direct `runVerification()` test genuinely exercises that branch; (3) the
reason-string centralization didn't introduce any typo/mismatch between
definition and usage sites; (4) no new scope crept in beyond these four
items. If you find the substance of this diff sound with no new defect,
approve at your genuine confidence level — do not artificially withhold
confidence for issues already resolved in prior rounds' critique, and do
not re-litigate the out-of-scope items (`submitDraft`'s retry-verification
path staying exit 2, clock-skew tolerance) that both this round's
instructions and Opus's own round-3 review explicitly scoped out.
