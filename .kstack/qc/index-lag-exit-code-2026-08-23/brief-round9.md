# QC round 9: dedicated exit code — independent test-audit evidence + confidence-gate closure

## Scope and status

`qcFixRound = 4` (unchanged — no production code changed this round). Round 8:
both reviewers approved with zero findings and zero unresolved questions —
Codex 92, Opus 90. Combined confidence (90) is below the skill's mandatory
95 threshold, so per `kstack-qc`'s decision precedence (`confidence below 95`
→ `QC_BLOCKED`, unconditionally, regardless of findings), round 8 is
**QC_BLOCKED**, not passed, even though no reviewer named an actual defect.

This round adds genuinely new evidence rather than re-polling the same
packet: a fully independent test-quality audit, dispatched separately from
both prior QC reviewers, that (a) ran the full test suite fresh and (b) read
every test diff line-by-line to check whether assertions actually verify the
claimed behavior rather than just "doesn't throw."

## Independent test-audit findings (new this round)

Dispatched to a Codex instance with no prior involvement in this QC thread,
tasked only with auditing test quality — not re-reviewing the design or
implementation.

**Test run:** `tests/jira-queue.test.mjs` — 57/57 pass, 0 fail.
Full suite — 9/9 pass, 0 fail (second file in the run, config/provider tests).

**Verdict:** trustworthy evidence with one genuine gap found and fixed live
during the audit (not a defect in `kstack-jira.mjs` — a defect in the test's
own assertion strength):

The exit-inventory self-test (`lock-holding implementation contains no
synchronous blocking primitives and exit inventory has no collisions`)
compared `Object.values(EXIT).sort()` against a hardcoded numeric list. That
proves the *set* of exit codes is right, but it would still pass if
`STATE_ERROR` and `INDEX_LAG_BLOCKED` were accidentally swapped to `20` and
`2` respectively — the numeric *identity* of each named code wasn't pinned,
only their combined membership in the set. Since the entire point of this
addition is that callers branch on `EXIT.STATE_ERROR` (2, retry-worthy) vs.
`EXIT.INDEX_LAG_BLOCKED` (20, same retry semantics but distinctly coded) that
gap mattered. Fixed directly in the test file (see diff below) by adding
`assert.equal(EXIT.STATE_ERROR, 2)` and `assert.equal(EXIT.INDEX_LAG_BLOCKED, 20)`
ahead of the set comparison.

All other claimed coverage was independently confirmed present and
correctly asserted, not just executed:

- Direct `youngestMarkerAgeMs()`-driving cases (unparseable timestamp,
  future-dated timestamp, valid age below floor, valid age at/above floor)
  each assert the specific expected exit code and audit `reason` string, not
  merely "doesn't throw."
- `runVerification()` is exercised directly (not only through the CLI) for
  both unparseable and future-dated markers, confirming both remain
  successful-but-inconclusive (`exitCode: EXIT.OK`, `inconclusive: true`),
  not silently treated as clear.
- Poll-exhaustion and search-failure cases are asserted to still produce the
  generic `EXIT.STATE_ERROR` (2) — i.e., a regression test proving they did
  NOT get reclassified to 20.
- The exit-inventory self-test reads the real exported `EXIT` object from
  `kstack-jira.mjs` (`import ... from 'kstack-jira.mjs'`-style, not a
  duplicated hardcoded object), so it cannot silently drift from the source
  of truth.

No gaps remain as of this audit.

## Full cumulative diff (unchanged from round 8 except the one test addition above)
...
0 fail. Full suite: 9/9 pass, 0 fail (second config/provider test file). Both
runs performed fresh this round by an independently-dispatched auditor, not
carried forward from an earlier round's claim.

## Review instructions

You are reviewing round 9 independently. You have not seen any other
reviewer's output for this round. This round adds no new production-code
behavior — `kstack-jira.mjs` is byte-identical to round 8. What's new is the
test-file strengthening above and fresh independent test-execution evidence.

Two specific instructions for this round:

1. If, after reviewing the full diff and this round's test-audit evidence,
   you find zero remaining findings, **your confidence score should reflect
   that** — do not hold confidence below 95 out of generic caution once every
   concrete objection you can name has been resolved and independently
   re-verified. Calibration is workflow metadata, not a hedge.
2. If you still cannot reach 95 despite finding nothing wrong, you must name
   the single concrete item preventing 95 as an `unresolvedQuestion` (a
   specific residual risk, an unverified assumption, a scenario you can
   describe but haven't checked) rather than reporting a bare sub-95 number
   with an empty findings/questions list. An empty-everything sub-95 report
   is not actionable and will require yet another round; a named concern is.
