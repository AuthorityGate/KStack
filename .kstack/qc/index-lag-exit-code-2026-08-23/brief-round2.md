# QC round 2: dedicated exit code for "blocked pending Jira search-index lag"

## Scope and status

Round 1 of independent dual QC (Codex confidence 91/revise, Opus confidence
62/revise) both converged on the SAME real defect: markers with a
missing/unparseable timestamp were misclassified under the new transient
"index-lag" exit code (20) instead of the generic "needs attention" exit
code (2), because the age-computation helper returned `0` (not an
error/Infinity sentinel) on parse failure, and `0` is always younger than
the 30-second floor — making a permanently-stuck case look retryable
forever. This is round 2: the fix for that exact defect, for fresh
independent re-review. `qcFixRound = 1` (first counted remediation round
after the original QC found a genuine defect).

This remains public-contract + security-adjacent work (per
`kstack-qc/SKILL.md`), so it requires both Codex and Opus again — the
implementer may be one of them but self-review alone does not pass.

## Round-1 critique (both reviewers, verbatim, condensed to the defect)

**Codex round 1:** "The missing-timestamp test now expects exit 20, but a
marker with no timestamp is not demonstrably younger than the 30-second
floor. It can remain blocked indefinitely, contradicting the defined
meaning that exit 20 applies solely while waiting for index lag."

**Opus round 1:** "A timestampless marker has no measurable age; if
youngestMarkerAgeMs treats it as age 0 it can never satisfy the floor, so
the CLI would be telling callers 'wait and retry' about a draft that is
permanently stuck and actually needs human repair — the exact opposite of
what the new code is supposed to signal, and a regression from the generic
exit 2 that correctly meant 'operator intervention required'."

Both also raised secondary findings (help-text completeness/precision,
submit test coverage strength, an exit-2-consumer survey) — see "What must
be true, round 2" below for how each was addressed.

## The round-2 fix

`youngestMarkerAgeMs()` (`plugins/kstack/scripts/kstack-jira.mjs`) now
returns `null` (not `0`) when the age is unmeasurable — an invalid clock, or
any marker with an unparseable `at` timestamp — while still returning
`Infinity` for the pre-existing empty-markers case, and a real millisecond
age otherwise. Its two callers were both updated:

- `preMutationDuplicateGate()`: `youngestMarkerAge === null` now audits
  `verify-inconclusive` with `reason: 'marker-age-unmeasurable'` and returns
  `{ blocked: true, unavailable: true, reason: 'search-unavailable' }` —
  routing to `EXIT.STATE_ERROR` (2), not `EXIT.INDEX_LAG_BLOCKED` (20).
  Only a genuinely measurable age below the floor now reaches
  `reason: 'index-lag'` (exit 20).
- `runVerification()`: same unmeasurable-vs-measurable distinction applied
  to its own `canClear`/audit-reason logic (this path's exit code was
  already `EXIT.OK` either way per the original design — only the recorded
  audit `reason` field changes, correctly distinguishing
  `'marker-age-unmeasurable'` from `'marker-younger-than-index-lag-minimum'`
  for operator visibility, per Codex's own item-1 scoping instruction to fix
  this consistently everywhere `youngestMarkerAgeMs()` is used without
  changing exit-code behavior where none was asked for).

## What must be true, round 2 (in addition to everything from round 1's brief, unchanged)

- A marker with a **measurable** age below the 30s floor → exit 20
  (`reason: 'index-lag'`), exactly as round 1 intended.
- A marker with an **unmeasurable** age (missing/unparseable timestamp) →
  exit 2 (`reason: 'search-unavailable'`, audited as
  `'marker-age-unmeasurable'`), NOT exit 20 — this is the round-1 defect,
  now fixed.
- The previously-flipped test ("pre-mutation duplicate gate blocks a marker
  with a missing timestamp") is reverted to asserting `EXIT.STATE_ERROR`,
  with an explanatory comment.
- A new explicit test proves BOTH branches side by side: measurable-and-young
  → 20; unmeasurable → 2.
- Submit's index-lag test now also asserts the resulting draft state and
  audit event, not just exit code and attempt count (Codex's round-1 ask).
- Both documentation sites (`JIRA_QUEUE.md`, in-code CLI help text) now: (a)
  explicitly name poll-exhaustion, search-failure, AND unmeasurable-age as
  all still exit 2; (b) narrow the exit-20 description to the specific
  pre-mutation duplicate gate rather than implying it covers every
  index-lag-adjacent wait (submit's separate retry-verification-interval
  wait is explicitly called out as intentionally still exit 2, out of the
  owner-approved scope).
- Consumer survey (Opus's round-1 ask): implementer reports no other
  repository location branches on Jira exit code 2 specifically — the
  `kstack-jira` skill only invokes the offline `draft` command, no wrapper
  script inspects the Jira CLI's exit code, and `.github/workflows/ci.yml`
  only runs `npm test`. Independently spot-check this claim if you have
  reason to doubt it; the packet cannot re-run a repo-wide grep for you
  under packet-only rules, so treat this specific claim's evidentiary
  weight accordingly (this is the implementer's self-report, not
  independently reproduced by either reviewer or by this orchestrating
  session).

## Verification evidence (implementer-reported for round 2)
...
`git diff --check` also reported clean (no whitespace-only diff issues).

## Full cumulative diff (complete, no elision — every changed line across all 3 files, base commit to current state, includes both round 1 and round 2 changes)
...
@@ -1055,29 +1056,29 @@ function verifyClearMinimumAgeMs(state) {
...
@@ -1097,25 +1098,29 @@ export async function runVerification(lock, draft, credentials, options = {}) {
   }
...
+        reason: markerAgeMeasurable ? 'marker-younger-than-index-lag-minimum' : 'marker-age-unmeasurable',
+        minimumMarkerAgeMs
+      } : {})
     }, lock.state.clock);
     await guardedWriteDraft(lock, draft);
...
@@ -1126,50 +1131,55 @@ async function preMutationDuplicateGate(lock, draft, credentials) {
...
+    queueAudit(draft, 'verify-inconclusive', { reason: 'marker-age-unmeasurable', minimumMarkerAgeMs }, lock.state.clock);
+    await guardedWriteDraft(lock, draft);
+    return { blocked: true, unavailable: true, reason: 'search-unavailable' };
+  }
...
@@ -1182,21 +1192,21 @@ async function submitDraft(state, id, args = {}) {
     }
...
@@ -1462,21 +1472,21 @@ async function resolveDraft(state, id, args = {}) {
 }
...
@@ -645,52 +648,130 @@ test('pre-mutation duplicate gate uses the injected clock on both sides of the 3
...
@@ -997,21 +1078,21 @@ test('ambiguous-history override remains independent from duplicate dismissal',
   writeDraft(state, draft);
...
This is a re-review, not a first pass — focus specifically on:

1. Does the `null`-vs-`Infinity`-vs-number contract change to
   `youngestMarkerAgeMs()` correctly handle every call site? Are there any
   OTHER callers of `youngestMarkerAgeMs()` beyond the two shown in this
   diff that might not have been updated (search the full diff/context for
   every reference)?
2. Does the fix actually close the exact gap both round-1 reviews
   identified, without introducing a new one (e.g., does `Infinity` ever
   get compared against the floor in a way that could misbehave now that
   `null` is a distinct third value)?
3. Are the round-1 secondary findings (help-text completeness/precision,
   submit test strength) genuinely addressed, not just cosmetically touched?
4. Is the consumer-survey claim (no other exit-2 consumers) plausible given
   what's visible in this packet, or does it need independent verification
   before this can pass?
5. Confirm no scope creep: still no changes to duplicate-detection, polling,
   or the marker-age floor VALUE — only classification of measurable vs.
   unmeasurable age.
