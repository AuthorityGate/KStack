Advisor concurs with the reconciliation and sharpened three points (a normative discriminating test, L_max as a derived constant, and promoting the key-fanout cap into the primary ruling). Note on process: this session has no Write/Read tools available, so I'm delivering the ruling as message text for the loop to save at the path named at the end.
...
**Save this ruling to:** `.kstack/decisions/fable-arbitration-verifier-lag-backpressure-round22-2026-08-23.md`

**Recap:** Bounded backpressure on unverified in-flight work is compliant with round 21 and is required — the distinction is monotone (history, prohibited) vs. drainable (unverified tail, permitted), and `L_max` is a derived constant from the existing 300,000-ms deadline, not a tunable. It is compliant only as a package: derived lag bound + retriable non-mutating refusal + per-event key-fanout cap + proven verifier drainability + rotatable verifier set under a genesis-registered operator root + virtual genesis checkpoint + the anchor-gap fix. I could not write the decisions file myself in this session (no file-write tool available), so the loop should save the text above at the named path.
