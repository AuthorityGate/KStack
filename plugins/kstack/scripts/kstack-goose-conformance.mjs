...
export class GooseConformanceError extends Error {
  constructor(code) { super(code); this.name = 'GooseConformanceError'; this.code = code; }
}

function fail(code) { throw new GooseConformanceError(code); }
...
export const GOOSE_OPERATION_FAMILIES = Object.freeze([
  'ADVISORY', 'ASK_REVIEWER', 'BACKGROUND', 'PRIVILEGED_LOCAL_MUTATION',
  'PRIVILEGED_SIDE_EFFECT', 'PUBLIC_MCP', 'REPOSITORY_WRITE'
]);
export const GOOSE_DEPENDENCY_STATUSES = Object.freeze(['SATISFIED', 'MISSING', 'STALE', 'MISMATCH', 'NOT_IMPLEMENTED']);
export const GOOSE_FIXTURE_GROUPS = Object.freeze([
...
export const GOOSE_ELIGIBILITY_STATUSES = Object.freeze(['FULL', 'DEGRADED_REGISTERED', 'QUARANTINED', 'UNSUPPORTED']);
...
const IDENTITIES = Object.freeze({
  dependencyGate: ['kstack.goose-conformance-dependency-gate.v1', 'KSTACK-GOOSE-CONFORMANCE-DEPENDENCY-GATE-V1'],
  dependencyGateSet: ['kstack.goose-conformance-dependency-gate-set.v1', 'KSTACK-GOOSE-CONFORMANCE-DEPENDENCY-GATE-SET-V1'],
  plan: ['kstack.goose-conformance-plan.v1', 'KSTACK-GOOSE-CONFORMANCE-PLAN-V1'],
  fixture: ['kstack.goose-conformance-fixture.v1', 'KSTACK-GOOSE-CONFORMANCE-FIXTURE-V1'],
  fixtureSet: ['kstack.goose-conformance-fixture-set.v1', 'KSTACK-GOOSE-CONFORMANCE-FIXTURE-SET-V1'],
  execution: ['kstack.goose-conformance-execution.v1', 'KSTACK-GOOSE-CONFORMANCE-EXECUTION-V1'],
  observerReceipt: ['kstack.goose-conformance-observer-receipt.v1', 'KSTACK-GOOSE-CONFORMANCE-OBSERVER-RECEIPT-V1'],
  evidenceSet: ['kstack.goose-conformance-evidence-set.v1', 'KSTACK-GOOSE-CONFORMANCE-EVIDENCE-SET-V1'],
  status: ['kstack.goose-operation-status.v1', 'KSTACK-GOOSE-OPERATION-STATUS-V1']
});
...
export function requiredGooseDependencies(operationFamily) {
  enumeration(operationFamily, GOOSE_OPERATION_FAMILIES, 'KSTACK_GOOSE_OPERATION_FAMILY_INVALID');
  return Object.freeze([...new Set([...BASE_DEPENDENCIES, ...EXTRA_DEPENDENCIES[operationFamily]])].sort());
}

export function validateGooseDependencyGate(value) {
  const code = 'KSTACK_GOOSE_DEPENDENCY_GATE_INVALID';
  exact(value, [
  ...
  enumeration(value.status, GOOSE_DEPENDENCY_STATUSES, code);
  registry(value.reasonCode, code);
  if (value.reasonCode !== DEPENDENCY_REASON[value.status]) fail(code);
  return immutable(value);
}

export function classifyGooseDependencyGate(input) {
  const code = 'KSTACK_GOOSE_DEPENDENCY_OBSERVATION_INVALID';
  exact(input, [
  ...
  return validateGooseDependencyGate({
...
export function evaluateGooseDependencyGateSet(input) {
  const code = 'KSTACK_GOOSE_DEPENDENCY_GATE_SET_INVALID';
  ...
  enumeration(input.operationFamily, GOOSE_OPERATION_FAMILIES, code);
  ...
  const rows = input.rows.map(validateGooseDependencyGate);
  const itemIds = rows.map((row) => row.hpItemId);
  if (!same(itemIds, [...itemIds].sort()) || new Set(itemIds).size !== itemIds.length) fail(code);
  const requiredItemIds = requiredGooseDependencies(input.operationFamily);
  if (!same(itemIds, requiredItemIds)) fail('KSTACK_GOOSE_DEPENDENCY_GATE_INCOMPLETE');
...
export function validateGooseDependencyGateSet(value) {
  const code = 'KSTACK_GOOSE_DEPENDENCY_GATE_SET_INVALID';
  ...
  const evaluated = evaluateGooseDependencyGateSet({
...
export function sealGooseConformancePlan(input, dependencyGateSetInput) {
  const code = 'KSTACK_GOOSE_PLAN_INVALID';
  exact(input, PLAN_FIELDS.filter((field) => !['schemaId', 'schemaVersion'].includes(field)), code);
  if (input.hostId !== 'goose') fail(code);
  ...
  if (input.plannedAt >= input.expiresAt) fail('KSTACK_GOOSE_PLAN_EXPIRED');
  const gateSet = validateGooseDependencyGateSet(dependencyGateSetInput);
  if (!gateSet.executable) fail('KSTACK_GOOSE_DEPENDENCIES_UNSATISFIED');
...
export function validateGooseConformancePlan(value) {
  const code = 'KSTACK_GOOSE_PLAN_INVALID';
  exact(value, PLAN_FIELDS, code); validateHead(value, 'plan', code);
  if (value.hostId !== 'goose') fail(code);
  ...
  if (value.plannedAt >= value.expiresAt) fail('KSTACK_GOOSE_PLAN_EXPIRED');
  return immutable(value);
}

export function validateGooseFixture(value) {
  const code = 'KSTACK_GOOSE_FIXTURE_INVALID';
  exact(value, [
  ...
  enumeration(value.fixtureGroupId, GOOSE_FIXTURE_GROUPS, code);
...
export function sealGooseFixtureSet(input) {
  const code = 'KSTACK_GOOSE_FIXTURE_SET_INVALID';
  exact(input, ['operationProfileDigest', 'requiredGroupIds', 'fixtures'], code);
  digest(input.operationProfileDigest, code);
  sortedUnique(input.requiredGroupIds, (value) => enumeration(value, GOOSE_FIXTURE_GROUPS, code), 1, GOOSE_FIXTURE_GROUPS.length, code);
  if (!Array.isArray(input.fixtures) || input.fixtures.length < 1 || input.fixtures.length > 1024) fail(code);
  const fixtures = input.fixtures.map(validateGooseFixture);
  ...
  for (const groupId of input.requiredGroupIds) {
    ...
    if (!polarities.has('POSITIVE') || !polarities.has('NEGATIVE')) fail('KSTACK_GOOSE_FIXTURE_COVERAGE_INCOMPLETE');
  }
...
export function validateGooseFixtureSet(value) {
  const code = 'KSTACK_GOOSE_FIXTURE_SET_INVALID';
  ...
  return sealGooseFixtureSet({ operationProfileDigest: value.operationProfileDigest, requiredGroupIds: value.requiredGroupIds, fixtures: value.fixtures }).fixtureSet;
}

export function advanceGooseExecution(currentState, nextState, facts = {}) {
  try { return advanceFixtureExecution(currentState, nextState, facts); }
  catch { fail('KSTACK_GOOSE_EXECUTION_TRANSITION_INVALID'); }
}

export function sealGooseExecution(value) {
  const code = 'KSTACK_GOOSE_EXECUTION_INVALID';
  exact(value, [
  ...
  if (value.state === 'PASS' && (!pass || value.failureCode !== null)) fail('KSTACK_GOOSE_EXECUTION_PASS_INVALID');
  if (value.state !== 'PASS' && value.failureCode === null) fail(code);
  if (value.actionBoundaryCrossed && !value.outcomeProven && value.state !== 'AMBIGUOUS') fail('KSTACK_GOOSE_EXECUTION_AMBIGUOUS');
  const execution = { ...head('execution'), ...value };
  return immutable({ execution, executionDigest: address('execution', execution) });
}

export function validateGooseExecution(value) {
  const code = 'KSTACK_GOOSE_EXECUTION_INVALID';
  ...
  return sealGooseExecution(Object.fromEntries(Object.entries(value).filter(([key]) => !['schemaId', 'schemaVersion'].includes(key)))).execution;
}

export const GOOSE_OBSERVER_KINDS = Object.freeze([
...
const ADVISORY_FIXTURE_STRATEGIES = Object.freeze({
  ...
  preservation: Object.freeze({
    POSITIVE: ['ALLOW', ['CODEX_BASELINE_MATCHED', 'CLAUDE_BASELINE_MATCHED', 'GOOSE_BASELINE_MATCHED']],
...
export function buildGooseAdvisoryFixtureSet(operationProfileDigest) {
  digest(operationProfileDigest, 'KSTACK_GOOSE_FIXTURE_SET_INVALID');
  const fixtures = GOOSE_FIXTURE_GROUPS.flatMap((fixtureGroupId) => ['NEGATIVE', 'POSITIVE'].map((polarity) => {
    ...
    return {
      ...
      preconditionDigest: hostAddress('KSTACK-GOOSE-ADVISORY-FIXTURE-PRECONDITION-V1', { fixtureGroupId, polarity }),
      inputArtifactDigests: [hostAddress('KSTACK-GOOSE-ADVISORY-FIXTURE-INPUT-V1', { fixtureGroupId, polarity })],
      expectedNativeEventSequence, expectedDecisionCode,
      observerExpectationDigests: [hostAddress('KSTACK-GOOSE-ADVISORY-OBSERVER-EXPECTATION-V1', { fixtureGroupId, polarity })],
      maximumPermittedSideEffectsDigest: hostAddress('KSTACK-GOOSE-ADVISORY-SIDE-EFFECT-BUDGET-V1', { fixtureGroupId, polarity, productionTargets: 0, credentials: 0 }),
      deadlineMs: fixtureGroupId === 'background-lifecycle' ? 30_000 : 20_000,
      cleanupContractDigest: hostAddress('KSTACK-GOOSE-ADVISORY-CLEANUP-V1', { fixtureGroupId, disposable: true, zeroOrphans: true }),
  ...
  return sealGooseFixtureSet({ operationProfileDigest, requiredGroupIds: [...GOOSE_FIXTURE_GROUPS], fixtures });
}

export function validateGooseObserverReceipt(value) {
  const code = 'KSTACK_GOOSE_OBSERVER_RECEIPT_INVALID';
  exact(value, [
  ...
  enumeration(value.observerKind, GOOSE_OBSERVER_KINDS, code);
...
export function sealGooseObserverReceipt(input) {
  const code = 'KSTACK_GOOSE_OBSERVER_RECEIPT_INVALID';
  exact(input, [
    'fixtureId', 'observerId', 'observerKind', 'observationDigest', 'observedAt',
    'owner', 'subjectWritable', 'available', 'contradicted'
  ], code);
  const observerReceipt = validateGooseObserverReceipt({ ...head('observerReceipt'), ...input });
  return immutable({ observerReceipt, observerReceiptDigest: address('observerReceipt', observerReceipt) });
}

export function adjudicateGooseFixture(input) {
  const code = 'KSTACK_GOOSE_FIXTURE_ADJUDICATION_INVALID';
  exact(input, [
  ...
  const fixture = validateGooseFixture(input.fixture);
  const plan = validateGooseConformancePlan(input.plan);
  ...
  const receipts = input.observerReceipts.map(validateGooseObserverReceipt);
...
export function sealGooseEvidenceSet(input) {
  const code = 'KSTACK_GOOSE_EVIDENCE_SET_INVALID';
  exact(input, [
  ...
  const plan = validateGooseConformancePlan(input.plan);
  const fixtureSet = validateGooseFixtureSet(input.fixtureSet);
  ...
  const executions = input.executions.map(validateGooseExecution);
  ...
  if (!same(expectedIds, actualIds) || new Set(actualIds).size !== actualIds.length) fail('KSTACK_GOOSE_EVIDENCE_MEMBERSHIP_INVALID');
  ...
  if (!same(input.observerReceiptDigests, observed)) fail('KSTACK_GOOSE_OBSERVER_MEMBERSHIP_INVALID');
  ...
  if (input.startMeasurementDigest !== input.endMeasurementDigest
    || executions.some((execution) => execution.environmentStartDigest !== input.startMeasurementDigest
      ...
      || execution.cleanupEvidenceDigest !== input.cleanupEvidenceDigest)) fail('KSTACK_GOOSE_ENVIRONMENT_CHANGED');
  timestamp(input.expiresAt, code);
  if (input.expiresAt !== plan.expiresAt) fail('KSTACK_GOOSE_EVIDENCE_EXPIRY_INVALID');
  const evidenceSet = {
...
export function deriveGooseOperationStatus(input) {
  const code = 'KSTACK_GOOSE_STATUS_INVALID';
  exact(input, [
  ...
  for (const [field, value] of Object.entries(input)) if (field.endsWith('Digest')) digest(value, code);
  timestamp(input.evaluatedAt, code); timestamp(input.expiresAt, code);
  if (input.evaluatedAt >= input.expiresAt) fail('KSTACK_GOOSE_STATUS_EXPIRED');
  ...
  const operationStatus = { ...head('status'), hostId: 'goose', maximumClaim: 'OPERATION_SCOPED_ONLY', ...input, status, reasonCodes };
