...
test('the central contract and source audit are bound without manifest self-reference', () => {
  ...
  assert.equal(manifest.entries.some((entry) => entry.path === 'secret-broker-accepted-design-v1.json'), true);
  assert.equal(manifest.entries.some((entry) => entry.path === 'schemas/secret-broker/v1/compatibility.schema.json'), true);
  assert.equal(manifest.entries.some((entry) => entry.path === 'schemas/secret-broker/v1/public-config.schema.json'), true);
  assert.equal(manifest.entries.some((entry) => entry.path === 'schemas/secret-broker/v1/public-request.schema.json'), true);
  assert.equal(manifest.entries.some((entry) => entry.path === 'schemas/secret-broker/v1/public-result.schema.json'), true);
  assert.equal(manifest.entries.some((entry) => entry.path === 'scripts/secret-broker/compatibility-v1.mjs'), true);
  assert.equal(manifest.entries.some((entry) => entry.path === 'scripts/secret-broker/config-v2.mjs'), true);
