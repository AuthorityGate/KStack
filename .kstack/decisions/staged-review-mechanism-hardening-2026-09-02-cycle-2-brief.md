...
Second, final acceptance fails closed on severity. Routing every structured
finding on an accepted final into bug-fix intake is right for defects that
implementation can absorb, but it is wrong for a high or critical security
finding: that would let a design leave the review loop carrying an unresolved
severe finding on the strength of a decision label. Acceptance now additionally
requires the absence of any high or critical security finding, lower severities
continue to route to intake unchanged, and the rule is defined once so the
runner and the independent gate cannot drift into disagreeing about whether a
given final was accepted.
...
Depends on: none
Acceptance intent: The primary releases only on an approval decision at or above the configured readiness confidence with all four structured finding arrays empty; exactly one final reviewer is dispatched and it is a different agent from the primary; the final receives a brief whose digest is identical to the one the primary received and receives no part of the primary's report; final acceptance requires a non-blocking decision at or above the configured final threshold and the absence of any unresolved high or critical security finding, so a design cannot leave the loop carrying one whatever the decision label or confidence says; lower-severity findings, failed checks, dissent, and open questions on an accepted final become mandatory bug-fix intake rather than vetoes, keeping acceptance and defect capture distinct without letting severity escape; and the severity rule is defined once and read by both the runner and the independent gate, so an accepted manifest and a blocking gate cannot disagree.
...
- Final acceptance was severity-agnostic, so an accepted final could carry a
  high or critical security finding and still authorize acceptance. This was
  confirmed to be real rather than a wording gap: the acceptance predicate
  tested only decision and confidence, and the independent gate explicitly
  exempted the final reviewer from its zero-security-findings rule. BLK-PROTOCOL
  now requires the absence of high and critical findings for acceptance, keeps
  lower severities on the intake path, and the rule is defined once so runner
  and gate read the same predicate.
- The isolation measurement was credited with cross-run non-observability while
  measuring only local writes from one dispatch. The claim is narrowed to what
  the method establishes — write-side non-persistence of review content — and
  BLK-ISOLATION's boundary now explicitly disclaims read-side and
  provider-service coverage and disclaims owning primary/final isolation, which
  BLK-DISCLOSURE owns through deletion ordering and the absence of a filesystem
  read primitive.
- Measurement identity was described as resolved binaries and reported version,
  which read as path-and-label matching that an in-place replacement could
  defeat. The description was inaccurate rather than the mechanism: identity
  already includes content digests of the resolved executable and launcher.
  BLK-ISOLATION now says so.
- Owner liveness distinguished a recycled process identifier only by command
  line, so a recycled identifier running the same command still read as live and
  could deny review indefinitely. Identity now also includes the process start
  time, which is assigned at creation and not reused, with unobservable
  attributes still treated conservatively as live.
- Recovery after dispatch but before durable publication was undefined. Result
  publication is now atomic, so a cycle either published a complete result or
  published none; an interrupted cycle is abandoned rather than resumed or
  advanced, is not part of the chain, and is re-run at the same cycle number,
  with the single-flight lock preventing a concurrent second dispatch and the
  consumed capacity charged manually.
