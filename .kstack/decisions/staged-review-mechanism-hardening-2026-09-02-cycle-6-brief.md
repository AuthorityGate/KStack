...
Acceptance intent: A chained cycle is admitted only when the presented prior brief hashes to the digest the prior result recorded, that brief states the same objective as the current one, the named thread agrees wherever the prior result records one, and the prior cycle number is exactly one less; a prior result carrying no cycle accounting cannot be chained; a cycle declaring no predecessor is a first cycle and carries no prior findings; result publication is atomic, so a cycle either published a complete result or published none and there is no partial state a successor could chain to; a cycle interrupted before publication is therefore abandoned rather than resumed, is not part of the chain, and is re-run from the beginning at the next cycle number rather than its own, because the number it occupied was spent on invocations no result can reuse; the admissible number is one past the furthest the thread actually reached, which is the later of the last published cycle and the last cycle that dispatched without publishing, so an interrupted cycle always has exactly one admissible successor and the chain never reaches a state where neither its own number nor the following one can be admitted; and the property is stated as consistency between presented artifacts, never as authenticity of them.
...
gate held it at confidence 48. Three of its four failed checks were defects
introduced by the prior cycle's own edits rather than pre-existing gaps, and all
three are closed here:

- The recovery model was internally non-executable. The dispatch log refused an
  interrupted cycle's own number while the chain rule required a published
  predecessor at exactly one less, so after an interruption neither that number
  nor the next was admissible. The admissible number is now one past the furthest
  the thread actually reached — the later of the last published cycle and the
  last cycle that dispatched without publishing — so an interrupted cycle always
  has exactly one admissible successor. The deadlock is covered by a test that
  builds the real sequence: a completed cycle, an interrupted dispatched cycle,
  then the refusal of the spent number and the admission of the next.
- BLK-CHAIN still described an interrupted cycle as re-run at its own number
  while BLK-BUDGET described the opposite. BLK-CHAIN now states the same rule and
  its reason: a charge that does not advance the number cannot affect a budget
  that compares the number to its maximum.
- BLK-ISOLATION asserted both that evidence records no dispatched review content
  reaching shared configuration state and that the measurement observes only one
  literal value. The broader clause is removed; the block now states the scan's
  result at the strength the scan supports, once.
- The measurement's inability to detect transformed, indirectly stored, or later
  readable content was raised as a finding against the narrowed claim. That scope
  is what this method establishes rather than a defect in it, and it is recorded
  out of band as an accepted limitation.
- Whether relayed, agent-recorded acceptances constitute owner acceptance was
  raised as unresolved. The design no longer puts that question to a reviewer:
  governance acceptances are recorded out of band and are not part of the
  reviewed set, so the design states its limits without asking anyone to ratify
  who accepted them.
...
Four residual limitations of this design are recorded out of band, in
`.kstack/decisions/staged-review-mechanism-hardening-2026-09-02-accepted-limitations.md`:
reduced independent coverage during repair cycles, the aggregator limit on
provider-family separation, attestation-only disposition authority, and the
literal-persistence scope of the isolation measurement. That record is a
governance artifact rather than part of this design; it is not submitted for
review and no question about it is put to a reviewer. It is named here so the
blocks below can state each limit plainly without the design appearing to leave
it unaccounted for. The blocks state what the mechanism does and does not
establish; they do not claim any limit is thereby harmless.

The tradeoff behind the first of those — that concentrating the second
