...
export function runUnicode16Oracle() {
  if (process.versions.unicode !== '16.0') throw new Error('KSTACK_REFLEXION_UNICODE_ORACLE_VERSION');
  ...
  const sourceDigest = crypto.createHash('sha256').update(JSON.stringify(UNICODE_ORACLE_SOURCES)).digest('hex');
  const record = {
