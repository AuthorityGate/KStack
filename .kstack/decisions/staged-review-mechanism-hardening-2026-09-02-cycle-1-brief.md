...
Depends on: none
Acceptance intent: The primary releases only on an approval decision at or above the configured readiness confidence with all four structured finding arrays empty; exactly one final reviewer is dispatched and it is a different agent from the primary; the final receives a brief whose digest is identical to the one the primary received and receives no part of the primary's report; final acceptance is a non-blocking decision at or above the configured final threshold, which no other block may raise, lower, or bypass; and structured findings on an accepted final become mandatory bug-fix intake rather than vetoes, so acceptance and defect capture are distinct outcomes.
...
Outcome: A provider build whose isolation behavior has not been observed cannot be dispatched.
Boundary: Owns the state-suppression precondition and the measurement that admits a build; owns no provider-internal behavior and no claim beyond the builds actually measured.
Depends on: BLK-DISCLOSURE
Acceptance intent: The flags that suppress cross-run persistence are a named set that constructing a spawn refuses to omit, so silently dropping one fails closed rather than degrading quietly; a dispatch is admitted only when recorded evidence matches the active backend's resolved binaries and its reported version and records that no dispatched review content reached the shared configuration directory; a build with no matching measurement dispatches nobody and charges nothing; measurement observes only the configuration files a real dispatch actually modified, using a probe value that never enters an operator-visible record so a later scan cannot match its own history; and the standing cost — every provider upgrade requires re-measurement before the next cycle — is accepted rather than worked around.
...
This thread succeeds `staged-primary-final-review-workflow-2026-08-29`, whose
eighth primary review assessed this same mechanism under an objective that also
carried the high-water requirement. That review's findings and this design's
answers:
...
- The core workflow contract was asserted in prose but owned by no block, so the
  ...
  outcome. BLK-PROTOCOL now owns it.
- BLK-PROTOCOL's first statement of final acceptance omitted what happens to
  structured findings on an accepted final, which read as though findings were
  either impossible or silently ignored. It now states the actual rule:
  acceptance is a non-blocking decision at or above the final threshold, and
  structured findings on an accepted final become mandatory bug-fix intake.
- Cross-run isolation depended on provider behavior with no requirement that the
  active build had been measured, so an unmeasured upgrade could restore
  persistence. BLK-ISOLATION now makes the measurement an admission gate bound
  to the resolved binaries and reported version, failing closed on any change.
- The brief carried reviewer-directed control text: it told the reviewer how to
  read a deferral and tried to narrow the assessment scope. Both are removed.
  Scope is stated once, as a boundary in the objective trace, and the brief
  states no review question of its own.
- Objective traceability failed because the brief deferred the objective's
  headline requirement. That is resolved by scope rather than by wording: this
  thread's objective is the mechanism, high-water preservation is out of scope
  and remains under KSTK-103, and the two are no longer bound to one digest.
