...
import {
  ...
  validateOpenCodeConformancePlan,
...
const evidencePath = evidenceArgument ? path.resolve(evidenceArgument) : path.join(qualificationRoot, 'opencode-v1.18.25-conformance-evidence.json');
const binaryPath = binaryArgument ? path.resolve(binaryArgument) : '/tmp/kstack-opencode-v1.18.25/bin/opencode';
...
function fail(message) { throw new Error(`KSTACK_OPENCODE_CONFORMANCE_EVIDENCE_INVALID: ${message}`); }
...
if (report.schema !== 'kstack-opencode-v1.18.25-protected-conformance-v1') fail('schema');
if (report.sourceEvidence.releaseTag !== 'v1.18.25'
    || report.sourceEvidence.binarySha256 !== 'd91e0d33676d0839f7cde87924cd4127ea88c9d6784eea9f009a7d08bdc60eeb') fail('pinned release identity');
if (sha256Hex(fs.readFileSync(binaryPath)) !== report.sourceEvidence.binarySha256) fail('binary bytes');
if (report.sourceEvidence.provenanceDigest !== fileDigest('.kstack/qualifications/opencode-v1.18.25-provenance.json')) fail('provenance drift');
...
if (report.isolation.environmentFacts.adapterDigest !== fileDigest('plugins/kstack/scripts/kstack-opencode-adapter.mjs')
    || report.isolation.environmentFacts.conformanceDigest !== fileDigest('plugins/kstack/scripts/kstack-opencode-conformance.mjs')
    || report.isolation.environmentFacts.providerScriptDigest !== fileDigest('.kstack/qualifications/opencode-v1.18.25-conformance-provider.mjs')
    || report.isolation.environmentFacts.pid1ReaperSourceDigest !== fileDigest('.kstack/qualifications/kstack-pid1-reaper.c')) fail('adapter/conformance/provider/reaper drift');

const gate = validateOpenCodeDependencyGateSet(report.dependencyGateSet);
if (!gate.executable || gate.rows.some((row) => row.status !== 'SATISFIED')
    || hostAddress('KSTACK-OPENCODE-CONFORMANCE-DEPENDENCY-GATE-SET-V1', gate) !== report.dependencyGateSetDigest) fail('dependency gate');
const plan = validateOpenCodeConformancePlan(report.plan);
if (hostAddress('KSTACK-OPENCODE-CONFORMANCE-PLAN-V1', plan) !== report.planDigest
    ...
    || plan.harnessDigest !== fileDigest('.kstack/qualifications/opencode-v1.18.25-conformance-child.mjs')
    || Date.parse(plan.expiresAt) <= Date.now()) fail('plan binding/currentness');
const fixtureSet = validateOpenCodeFixtureSet(report.fixtureSet);
const frozenFixtureSet = buildOpenCodeAdvisoryFixtureSet(plan.operationProfileDigest);
...
const executions = new Map(report.executions.map((execution) => [execution.fixtureId, validateOpenCodeExecution(execution)]));
const receipts = new Map(report.observerReceipts.map((receipt) => [receipt.fixtureId, validateOpenCodeObserverReceipt(receipt)]));
...
const observerReceiptDigests = report.observerReceipts.map((receipt) => hostAddress('KSTACK-OPENCODE-CONFORMANCE-OBSERVER-RECEIPT-V1', receipt)).sort();
...
const status = deriveOpenCodeOperationStatus(statusInput);
